...
class DOOracle extends JTable
{
	...
	{
		$sql = "SELECT RUT_FUNCIONARIO, DV_FUNCIONARIO FROM MEDISYN_1.CAC_FUNCIONARIOS 
						WHERE (VIGENCIA = 'S' AND TIPO_FUNCIONARIO = 'I') AND RUT_FUNCIONARIO = '$rut'";

		$this->conexion();
		$stid	= oci_parse($this->_link, $sql);
		$rtf = oci_execute($stid);

		$obj = new stdClass;
		$obj->rut = $rut;
		$obj->rutDv 		= 'FUNCIONARIO NO REGISTRADO BD BANMEDICA';
		$obj->linkFoto	= 'http://'.$_SERVER['HTTP_HOST'].'/personas-admin/img/avatars/user.jpg';

		while ($row = oci_fetch_assoc($stid)) {
			$obj->rutDv 		= $row['RUT_FUNCIONARIO'].$row['DV_FUNCIONARIO'];
			$ruta 					= "/var/www/html/portal/images/fotos/".$obj->rutDv."/".$obj->rutDv.".jpg";
			if(file_exists($ruta))
				$obj->linkFoto 	= 'http://'.$_SERVER['HTTP_HOST'].'/images/fotos/'.$obj->rutDv.'/'.$obj->rutDv.'.jpg';
			else
				$obj->linkFoto 	= 'http://'.$_SERVER['HTTP_HOST'].'/personas-admin/img/avatars/user.jpg';
		}
		return $obj;
	...
	function personas( $filtro=null )
	{
		...
		if( $_encontrados == '' || $_encontrados == 'S' ) :
			...
			while ( $row = oci_fetch_assoc( $funcionarios ) ) :
				...
				$dir = dirname(__FILE__).'/../../../../images/fotos/';
				$fotosDir = scandir($dir);

				foreach($fotosDir as $val){
					if($val[0] == ".") continue;
					if(is_dir($dir.$val)) {
						if(strstr($val, $row['RUT_FUNCIONARIO'])) {
							$row['FOTO']	= _DO_FOTOS_BASE.$val.'/'.$val.'.jpg';
						}
					}
				}
