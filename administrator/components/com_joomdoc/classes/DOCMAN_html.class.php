...
$pear_path = $docman->getPath('contrib', 'pear');
require_once $pear_path . 'HTML_Select.php';
require_once $docman->getPath('classes', 'user');
require_once $docman->getPath('classes', 'groups');
...
class dmHTML_UserSelect extends dmHTML_Select
{
  	...
  	function addUsers()
    {
        ...
        if ( !$docman->getCfg('individual_perm', 1))
        {
        	return;
        }
...
class dmHTML extends JHTML 
{
    ...
    {
        $docman = &DocmanFactory::getDocman();
        require_once($docman->getPath('classes', 'utils'));
        ...
        $parent = JHTML::_('select.genericlist',$options, 'catid', 'id="catid" class="inputbox" size="1" onchange="' . $action . '"', 'value', 'text', $id,null,false,false);
        return $parent;
    }
    ...
    function imageList($name, &$active, $javascript = null, $directory = null)
    {
        ...
        $imageFiles = DOCMAN_Compat::mosReadDirectory($root_path . $directory);
    ...
    function maintainerList(&$doc, $name, $attributes = null, $tabOffset = 0)
    {
    	...
    	if($dmuser->canAssignMaintainer($doc))
    	{
    		//create select list
    		$select =& new dmHTML_UserSelect($name, 1, $attributes, $tabOffset );
    		$select->addOption(_DML_SELECT_USER, _DM_PERMIT_NOOWNER);
    ...
    function uploadSelectList($method = '')
    {
        ...
        $allow_all = $dmuser->isSpecial ? true : false;

        if (! $allow_all) {
            $allowed = $docman->getCfg('methods' , array('http'));
        }
