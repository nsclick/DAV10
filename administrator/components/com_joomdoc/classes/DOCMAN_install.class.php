 ...
 * @version $Id: DOCMAN_install.class.php 561 2008-01-17 11:34:40Z mjaz $
 * @package JoomDOC
...
if(!function_exists('cleanupInstall')) {
    function cleanupInstall( $userfile_name, $resultdir) {
        ...
        if (file_exists( $resultdir )) {
            dm_deldir( $resultdir );
            unlink( DocmanFactory::getPathName( $mosConfig_absolute_path . '/media/' . $userfile_name, false ) );
        }
    }
}

function dm_deldir( $dir )
{
	...
	while (($entryname = readdir( $current_dir ))) {
		if ($entryname != '.' and $entryname != '..') {
			if (is_dir( $dir . $entryname )) {
				...
				$path = JPath::clean($dir . $entryname);
...
class DM_mosInstaller {
    ...
    */
    function extractArchive() {
        ...
        $base_Dir       = &DocmanFactory::getPathName( $mosConfig_absolute_path . '/media' );

        $archivename    = $base_Dir . $this->installArchive();
        $tmpdir         = uniqid( 'install_' );

        $extractdir     = &DocmanFactory::getPathName( $base_Dir . $tmpdir );
        $archivename    = &DocmanFactory::getPathName( $archivename, false );
		...
		$filesindir = array_merge($files, $folders);
		// Sort them all
		asort($filesindir);

        if (count( $filesindir ) == 1) {
            if (is_dir( $extractdir . $filesindir[0] )) {
                $this->installDir( DocmanFactory::getPathName( $extractdir . $filesindir[0] ) );
            }
        }
        return true;
    }
    ...
    */
    function parseFiles( $tagName='files', $special='', $specialError='', $adminFiles=0 ) {
        ...
        if ($folder = $files_element->getAttribute( 'folder' )) {
            $temp = &DocmanFactory::getPathName( $this->unpackDir() . $folder );
            if ($temp == $this->installDir()) {
                // this must be only an admin component
                $installFrom = $this->installDir();
            } else {
                $installFrom = &DocmanFactory::getPathName( $this->installDir() . $folder );
            }
        } else {
            $installFrom = $this->installDir();
        }
        ...
        if ($tagName == 'media') {
            ...
            $installTo = &DocmanFactory::getPathName( $mosConfig_absolute_path . '/images/stories' );
        } else if ($adminFiles) {
    ...
    function copyFiles( $p_sourcedir, $p_destdir, $p_files, $overwrite=false ) {
        if (is_array( $p_files ) && count( $p_files ) > 0) {
            foreach($p_files as $_file) {
                $filesource = &DocmanFactory::getPathName( DocmanFactory::getPathName( $p_sourcedir ) . $_file, false );
                $filedest   = &DocmanFactory::getPathName( DocmanFactory::getPathName( $p_destdir ) . $_file, false );
...
class DOCMAN_Installer extends DM_mosInstaller {
    ...
    function installPackage($p_fromdir = null)
    {
        ...
        if( !typecast($this, $classMap[$this->installType()]) ) {
        	$mainframe->redirect( 'index2.php?option=com_joomdoc', _DML_INSTALLER_ERROR.': Typecasting to '.$classMap[$this->installType()]);
        }
    ...
    function uninstallPackage($package, $type)
    {
        ...
        if( !typecast($this, $classMap[$type]) ) {
            $mainframe->redirect( 'index2.php?option=com_joomdoc', _DML_INSTALLER_ERROR.': Typecasting');
        }
...
class DOCMAN_InstallerTheme extends DOCMAN_Installer {
    ...
    */
    function install($p_fromdir = null)
    {
        ...
        $this->elementDir(DocmanFactory::getPathName($absolute_path
                 . '/components/com_joomdoc/views/themes/' . strtolower(str_replace(" ", "_", $this->elementName())))
            );
    ...
    */
    function uninstall($id)
    {
        ...
        // Delete directories
        $path = $absolute_path
         . '/components/com_joomdoc/views/themes/' . $id;

        $id = str_replace('..', '', $id);
        if (trim($id)) {
            if (is_dir($path)) {
                return dm_deldir(DocmanFactory::getPathName($path));
            } else {
