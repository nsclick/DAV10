 ...
 * @version $Id: DOCMAN_user.class.php 631 2008-02-25 19:13:33Z mjaz $
 * @package JoomDOC
...
class DOCMAN_User {
	...
	function canAccess() {
		...
		if (! $this->userid && $docman->getCfg ( 'registered' ) == _DM_GRANT_NO) {
			return 0;
		}
		// check if the component is down
		if (! $this->isSpecial && $docman->getCfg ( 'isDown' )) {
			return - 1;
		}
	...
	function canUpload() {
		...
		if ($this->userid) {
			$upload = $docman->getCfg ( 'user_upload' );
	...
	function canDownload($doc = null) {
		...
		if ($this->userid == 0 && $docman->getCfg ( 'registered' ) != _DM_GRANT_RX) {
			return false;
		}
	...
	function canPublish($doc = null) {
		...
		if ($this->userid) {
			$publish = $docman->getCfg ( 'user_publish' );
	...
	function canAssignViewer($doc = null) {
		...
		if ($docman->getCfg ( 'reader_assign' ) & _DM_ASSIGN_BY_AUTHOR) {
			if ($this->userid == $doc->dmsubmitedby) {
				return true;
			}
		}
		
		if ($docman->getCfg ( 'reader_assign' ) & _DM_ASSIGN_BY_EDITOR) {
			if ($this->canEdit ( $doc, false )) {
				return true;
			}
		}
	...
	function canAssignMaintainer($doc = null) {
		...
		if ($docman->getCfg ( 'editor_assign' ) & _DM_ASSIGN_BY_AUTHOR) {
			if ($this->userid == $doc->dmsubmitedby) {
				return true;
			}
		}
		
		if ($docman->getCfg ( 'editor_assign' ) & _DM_ASSIGN_BY_EDITOR) {
			if ($this->canEdit ( $doc, false )) {
				return true;
			}
		}
