...
class HTML_DMConfig
{
    function configuration(&$lists)
    {
        ...
        $mosConfig_live_site = $config->getValue('config.live_site');
        global $_DOCMAN;
        ...
        <div class="dm_filters">
            <span class="componentheading">configuration file:
			 <?php echo is_writable($mosConfig_absolute_path.DS.'administrator'.DS.'components'.DS.'com_joomdoc'.DS.'docman.config.php') ? '<b><font color="green">'._DML_WRITABLE.'</font></b>' : '<b><font color="red">'._DML_UNWRITABLE.'</font></b>' ?>
			</span>
        </div>
        ...
        <form action="index2.php?option=com_joomdoc&amp;task=saveconfig" method="post" name="adminForm" id="adminForm">
    ...
    <table class="adminform">
        ...
        <tr>
            ...
            <td>
                <?php
                $newpath = JPATH_ROOT.DS.'joomdocs';
                //$path = $_DOCMAN->getCfg('dmpath') ? $_DOCMAN->getCfg('dmpath') : $newpath;
				$path = $_DOCMAN->getCfg('dmpath') == $newpath ? '' : $path;
                ?>
    ...
    <table class="adminform">
        ...
        <tr>
            ...
            <td width="250"><?php echo $lists['default_viewer'];?></td>
