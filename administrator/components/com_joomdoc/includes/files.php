...
function showFiles()
{
    ...
    global $mainframe, $option, $section, $mosConfig_list_limit;
    global $_DOCMAN;
    ...
    for ($i = 0, $n = count($files);$i < $n;$i++)
    {
        ...
        $database->setQuery("SELECT COUNT(dmfilename) FROM #__joomdoc WHERE dmfilename='" . $database->getEscaped($file->name) . "'");
...
function removeFile($cid)
{
    ...
    foreach($cid as $name) {
        $database->setQuery("SELECT COUNT(dmfilename) FROM #__joomdoc WHERE dmfilename='" . $database->getEscaped($name) . "'");
        $result = $database->loadResult();
        ...
        if ($result != 0)
            $mainframe->redirect("index2.php?option=com_joomdoc&section=files", _DML_ORPHANS_LINKED);

        $file = $_DOCMAN->getCfg('dmpath') . "/" . $name;

        if (!unlink($file)) {
            $mainframe->redirect("index2.php?option=com_joomdoc&section=files", _DML_ORPHANS_PROBLEM);
        }
    }

    $mainframe->redirect("index2.php?option=com_joomdoc&section=files", _DML_ORPHANS_DELETED);
}
        ...
        case 2:
            switch ($method) {
                ...
                case 'ftp':
                    ...
                    break;
                case 'link':
                    $mainframe->redirect("index2.php?option=com_joomdoc&section=documents&task=new&makelink=1",_DML_CREATEALINK);
                ...
                case 'transfer':
                    ...
                    break;
                default:
                    $mainframe->redirect("index2.php?option=com_joomdoc&section=files", _DML_SELECTMETHODFIRST);
            }
            break;
        case 3:
            DOCMAN_token::check() or die('Invalid Token');
            switch ($method) {
                case 'http':
                    ...
                    if (!$result) {
                        $mainframe->redirect("index2.php?option=com_joomdoc&section=files", _DML_ERROR_UPLOADING . " - " . $upload->_err);
                    } else {
                        ...
                        if ($batch && $old_filename <> null) {
                            ...
                            if (!extension_loaded('zlib')) {
                                $mainframe->redirect("index2.php?option=com_joomdoc&section=files", _DML_ZLIB_ERROR);
                            }
                            ...
                            if (!$zip->extract($target_directory)) {
                                $mainframe->redirect("index2.php?option=com_joomdoc&section=files", _DML_UNZIP_ERROR);
                            }
                        ...
                        if ($old_filename) {
                            ...
                            $database->setQuery("UPDATE #__joomdoc SET dmfilename='". $database->getEscaped($result->name) ."' WHERE dmfilename='". $database->getEscaped($old_filename) ."'");
                        ...
                        $mainframe->redirect("index2.php?option=com_joomdoc&section=files&task=upload&step=4" . "&result=" . urlencode($result->name) . "&batch=" . (0 + $batch) . "&old_filename=" . $old_filename,
                ...
                case 'transfer':
                    ...
                    if ($result) {
                        ...
                        $mainframe->redirect("index2.php?option=com_joomdoc&section=files&task=upload&step=4" . "&result=" . urlencode($result->name) . "&batch=0&old_filename=1",
                            _DML_SUCCESS . ' &quot;' . $result->name . '&quot; - ' . _DML_FILEUPLOADED);
                    } else {
                        $mainframe->redirect("index2.php?option=com_joomdoc&section=files", $upload->_err);
                    }
                    break;
            }
            break;
