...
function showThemes()
{
    global $mainframe, $_DOCMAN;
    global $mosConfig_list_limit;

    $limit = $mainframe->getUserStateFromRequest('viewlistlimit', 'limit', $mosConfig_list_limit);
    $limitstart = $mainframe->getUserStateFromRequest("view{com_joomdoc}limitstart", 'limitstart', 0);

    $themesBaseDir = &DocmanFactory::getPathName($_DOCMAN->getPath('themes'));
    ...
    foreach($themesDirs as $themeDir) {
        $path = &DocmanFactory::getPathName($themesBaseDir . $themeDir);
...
function saveTheme()
{
    ...
    if ($config->saveConfig()) {
        if( $task == 'save' ) {
            $url = 'index2.php?option=com_joomdoc&section=themes';
        } else { // $task = 'apply'
            $url = 'index2.php?option=com_joomdoc&section=themes&task=edit&cid[0]='.$id;
        }
        $mainframe->redirect( $url, _DML_SAVED_CHANGES);
    } else {
        $mainframe->redirect("index2.php?option=com_joomdoc&section=themes", _DML_CONFIG_ERROR);
    }
}
...
function publishTheme($cid)
{
    ...
    $mainframe->redirect('index2.php?option=com_joomdoc&section=themes');
}
...
function removeTheme($cid)
{
    ...
    HTML_DMThemes::showInstallMessage('', 'Uninstall ' . $installer->filename . ' - Success' ,
        'index2.php?option=com_joomdoc&section=themes');
}
...
function uploadTheme($userfile)
{
    ...
    if (!(bool)ini_get('file_uploads')) {
        HTML_DMThemes::showInstallMessage(_DML_ENABLE_FILE_UPLOADS,
            _DML_INSTALLER_ERROR, 'index2.php?option=com_joomdoc&task=cpanel');
        exit();
    }
...
function installTheme($userfile)
{
    ...
    $path = &DocmanFactory::getPathName($userfile);
    if (!is_dir($path)) {
        $path = dirname($path);
    }
...
function editThemeCSS($p_tname)
{
    ...
    if ($fp = fopen($file, 'r')) {
        ...
        $mainframe->redirect('index2.php?option=com_joomdoc&section=themes', _DML_OPFAILED_COULDNT_OPEN . $file);
    }
}
...
function saveThemeCSS()
{
    ...
    $theme = trim(JRequest::getVar( 'theme', ''));
    ...
    if (!$theme) {
        $mainframe->redirect('index2.php?option=com_joomdoc&section=themes', _DML_OPFAILED_NO_TEMPLATE);
    }

    if (!$filecontent) {
        $mainframe->redirect('index2.php?option=com_joomdoc&section=themes', _DML_OPFAILED_CONTENT_EMPTY);
    }

    $file = $_DOCMAN->getPath('themes', $theme) . "/css/theme.css";

    if (is_writable($file) == false) {
        $mainframe->redirect('index2.php?option=com_joomdoc&section=themes',_DML_OPFAILED_UNWRITABLE);
    }
    ...
    if ($fp = fopen ($file, 'w')) {
        ...
        if( $task == 'save_css' ) {
            ...
            $url = 'index2.php?option=com_joomdoc&section=themes&task=edit_css&cid[0]='.$theme;
        }

        $mainframe->redirect( $url, _DML_SAVED_CHANGES);
    } else {
        $mainframe->redirect('index2.php?option=com_joomdoc&section=themes', _DML_OPFAILED_CANT_OPEN_FILE);
    }
}
