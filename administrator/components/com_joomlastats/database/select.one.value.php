...
class js_JSDbSOV extends js_JSDatabaseAccess
{
	...
	function getJSLastSummarizationDate( &$LastSummarizationDate_result ) {
		...
		$query = ''
		. ' SELECT'
		. '   MAX(CAST(CONCAT(s.year, \'-\', s.month, \'-\', s.day) AS date))'
		. ' FROM'
		. '   #__jstats_page_request_c s';
		$this->db->setQuery( $query );
	...
	/**
	 * Gets number of not identified visitors
	 *
	 * @todo I am not shure if we need to make join between 3 tables. - Reducing tables significaly increase performance in MySql
	 *
	 * @param in  bool    $include_summarized
	 * @param out integer
	 * @return true on success
	 */
	function selectNumberOfNotIdentifiedVisitors( $FilterDate, $include_summarized, &$NumberOfNotIdentifiedVisitors_result ) {
		...
		. '   a.tld = b.tld'
		. '   AND c.ip_id = a.id'
		. '   AND a.type = '._JS_DB_IPADD__TYPE_NOT_IDENTIFIED_VISITOR
		...
		$this->db->setQuery( $query );
		$NumberOfNotIdentifiedVisitors_result = $this->db->loadResult();
		if ($this->db->getErrorNum() > 0)
			return false;
	...
	function selectTotalNumberVisitorsWithBots( &$TotalNumberVisitorsWithBots_result ) {
		...
		$query = ''
		. ' SELECT'
		. '   SQL_BIG_RESULT COUNT(*) AS nbr_visitors_without_bots' //@todo we need SQL_BIG_RESULT? If Yes, other db queries should be fixed
		. ' FROM'
		. '   #__jstats_visits AS c'
		. '   LEFT JOIN #__jstats_ipaddresses a ON (c.ip_id=a.id)'
		;
		$this->db->setQuery( $query );
	...
	function selectNumberOfVisitors( $visitors_type, $include_summarized, $buid, $date_from, $date_to, &$NumberOfVisitors_result ) {
		...
		$query = ''
		. ' SELECT'
		. '   SQL_BIG_RESULT COUNT(*)' //@todo we need SQL_BIG_RESULT? If Yes, other db queries should be fixed
		. ' FROM'
		. '   #__jstats_visits AS v'
		. '   LEFT JOIN #__jstats_ipaddresses a ON (v.ip_id=a.id)'
		. ' WHERE'
		. '   '.( ($visitors_type!=='') ? 'a.type='.$visitors_type : '1=1' )
		. '   '.( ($include_summarized==false) ? 'AND v.id >= '.$buid : '')
		. '   AND '.$this->getConditionStringFromDates('v', $date_from, $date_to)
		;
		$this->db->setQuery( $query );
	 ...
	 * @param $include_summarized  values: true; false; //get this value from $JSConf->include_summarized; ($JSConf = new js_JSConf();)
	 * @param $buid				   required only if $include_summarized = false //get this value from $JSDbSOV->getBuid($buid); ($JSDbSOV = js_JSDbSOV();)
	 * @param $year                values: ''; '2009' //use '' for all years
	 * @param $month               values: ''; '06'; '6' //use '' for all months
	 * @param $day                 values: ''; '03'; '3' //use '' for all days
	 *
	 * NOTICE:
	 *   This function is faster than selectNumberOfVisitors() (we should check this condition in MySQL 5.x)
	 *
	 * @param out integer
	 * @return true on success
	 */
	function selectNumberOfVisitorsForYMD( $visitors_type, $include_summarized, $buid, $year, $month, $day, &$NumberOfVisitors_result ) {
		...
		. '   '.( ($visitors_type!=='') ? 'a.type='.$visitors_type : '1=1' )
		. '   '.( ($include_summarized==false) ? 'AND v.id >= '.$buid : '')
		. '   '.( ($year!=='') ? 'AND YEAR(v.time)='.$year : '' )
		. '   '.( ($month!=='') ? 'AND MONTH(v.time)='.$month : '' )
		. '   '.( ($day!=='') ? 'AND DAYOFMONTH(v.time)='.$day : '' )
		;
		$this->db->setQuery( $query );
		$NumberOfVisitors_result = $this->db->loadResult();
		if ($this->db->getErrorNum() > 0)
	 ...
	 *
	 * @todo I think query could be optimized (should be great gain)
	 * @bug? I am not sure if this query return correct results
	 *
	 * @param out integer
	 * @return true on success
	 */
	function selectTotalNumberPageImpressionsWithBotsWithSummarized( &$TotalNumberPageImpressionsWithBotsWithSummarized_result ) {
		...
		$query = 'SELECT SQL_BIG_RESULT SUM(s.count) FROM #__jstats_page_request_c AS s';
		$this->db->setQuery( $query );
