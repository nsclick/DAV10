...
class js_JSExcludeTpl
{
	...
	function viewJSExcludeManagerPageTpl( &$rows, $pagination, $search ) {
		...
		$FilterNextToTitleArr	= array();
		$FilterNextToTitleArr[] = '&nbsp;&nbsp;' . JTEXT::_( 'Search' ) . ':&nbsp;';
		$FilterNextToTitleArr[] = '<input type="text" name="search" id="search" value="'.$search.'" class="inputbox" size="30" onChange="submitbutton(\'js_view_exclude\');" />';

		//echo '<div style="text-align: left;"><!-- needed by j1.0.15 -->';//can not be used for lists! (in FF mosPageNav generate footer that is left justified instead of center)
		echo $JSTemplate->generateBeginingOfAdminForm( 'js_view_exclude' );
		echo '<input type="hidden" name="boxchecked" value="0" />' . "\n";

		echo $JSTemplate->generateHeaderIconAndTitleForJ10( 'JoomlaStats', JTEXT::_('Exclude Manager'), $FilterNextToTitleArr );

		if( js_getJoomlaVesrion_IsJ15x() == true ) {
			//filter should be at left side mic: what does this mean ???
			echo implode( '', $FilterNextToTitleArr );
		} ?>
