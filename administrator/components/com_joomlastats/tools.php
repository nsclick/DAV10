 ...
 */
class js_JSTools extends JSBasic
{
	...
	*/
	...
	function doJSUninstall() {
		...
		foreach( $JSSystemConst->allJSDatabaseTables as $db_table_name) {
			$this->db->setQuery('DROP TABLE `'.$db_table_name.'`');
			$this->db->query();
			if ($this->db->getErrorNum() > 0)
				$errors[] = $this->db->getErrorMsg().', '.$query;//@at in j1.5.x $query is relevant. SQL is inside ErrorMsg
		}
		...
		if( ( !$this->db->query() ) || ( count( $rowList ) != 1 ) || $bug_manualUninstal ) {
		...
		//mosRedirect( 'index2.php?option=com_installer&type=components&task=manage' );
		...
		//mosRedirect( 'index2.php?option=com_installer&type=components&task=remove&eid='.$JSComponentId.'&'.$token.'=1&boxchecked=1' );
	...
	{
		$last_month_ts = mktime(0, 0, 0, date("m")-1, date("d"), date("Y"));

		$last_month_str = date( 'Y-m-d', $last_month_ts );
		...
		;
		$this->db->setQuery( $query );
		$entries_to_summarize = $this->db->loadResult();

		$query = 'SELECT MIN(CAST(CONCAT(year, \'-\', month, \'-\', day) AS date))'
		. ' FROM #__jstats_page_request'
		;
		$this->db->setQuery( $query );
		$oldest_entry_date = $this->db->loadResult();
	...
	/** Previusly this code update row in #__jstats_page_request_c if such row exist - now it not could be done in this way because we removed hour - we summaraze for whole day)
	...
	function summarizeDbForParticularPageAndDay($page_id, $year, $month, $day, $sum)
	{
		...
		;
		$this->db->setQuery( $query );
		$current_sum = $this->db->loadResult();
		if ($this->db->getErrorNum() > 0)
			return false;
	 ...
	 * - copies consolidaded data in #__jstats_page_request_c
	...
	function doJSSummarize() {
		...
		;
		$this->db->setQuery( $query );
		$total_entries_to_summarize = $this->db->loadResult();
		...
		if( $this->_debug() ) {
			echo 'DEBUG info from JoomlaStats:<br />'
			. '<ul><li>start...</li>'
		...
		;
		$this->db->setQuery( $query, 0, $NrOfRecordsPossible );
		...
		if( $total_s > 0 ) {
			...
			require_once( dirname(__FILE__) .DS. 'res.jlprogressbar.php' );

			//JL
			//JLCoreApi::import('JLProgressbar', 2,0);
			$title		= JTEXT::_( 'Summarizing' );
			$succesUrl	= $this->_index() . '?option=com_joomlastats&task=js_view_summarize'; //is this url ever used? If Yes, when it is used?
			$errorUrl 	= $this->_index() . '?option=com_joomlastats&task=js_view_summarize'; //is this url ever used? If Yes, when it is used?
			...
			if( $this->_debug() ) {
				echo 'DEBUG Info from JoomlaStats - Summarizing process:<br />'
				. '<ul>'
			...
			foreach( $rows as $row ) {
				...
				if( $isSummarizationSuccesful == false ) {
					...
					if( isJ15() ) {
						...
						mosRedirect( 'index2.php?option=com_joomlastats&task=js_view_summarize', $msg );
					}
				}
				...
				if( $nbr_records_successfuly_transfered_s % $DisplayProcesBarPer == 0 ) {
					...
					if( $this->_debug() ) {
						echo '<b>Complete: ' . ((int)($complete*100)) . '%</b>';
						echo '&nbsp;&nbsp;&nbsp;&nbsp;';
						echo 'nbr_records_successfuly_transfered_s: ' . $nbr_records_successfuly_transfered_s . '&nbsp;&nbsp;&nbsp;';
						echo 'nbr_records_successfuly_transfered: ' . $nbr_records_successfuly_transfered . '&nbsp;&nbsp;&nbsp;';
						echo '<br/>';
					}
					//JL
		...
		$this->db->setQuery( $query );
		$this->db->query();
		$err_num += $this->db->getErrorNum();

		$query = 'OPTIMIZE TABLE `#__jstats_page_request_c`';
		$this->db->setQuery( $query );
		$this->db->query();
		$err_num += $this->db->getErrorNum();
	...
	function doGraphic() {
		...
		if( isJ15() ) {
			JToolBarHelper::title( 'JoomlaStats'.': <small><small>[ ' . JTEXT::_( 'Charts' ) . ' ]</small></small>',
			'js_js-logo.png' );
		}
