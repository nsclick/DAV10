...
class modMenuHelper
{
	...
	function buildMenu()
	{
		...
		$menuMenu			= true;
		$menuContenido		= true;
		$menuComponentes	= true;
		$menuExtensiones	= true;
		$menuHerramientas	= true;
		$menuAyuda			= true;
		
		$menuNoComponentes	= array();
		
		if( (int)$user->get('gid') == 24 ) :
			$canConfig			= false;
			$menuExtensiones	= false;
			$menuAyuda			= false;
			//$menuNoComponentes	= array("com_banners","com_contact","com_newsfeeds","com_search","com_poll","com_xmap","com_extplorer","com_joommyadmin");
			$menuNoComponentes	= array("com_jbolo", "com_do", "com_ccboard", "com_xmap", "com_contact", "com_joommyadmin");
		endif;
		...
		if( $menuComponentes ) :
			if ($editAllComponents)
			{
				...
				foreach ($comps as $row)
				{
					if( array_search( $row->option, $menuNoComponentes ) !== false ) :
						continue;
					endif;
