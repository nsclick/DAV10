...
class ContentHelperRoute
{
	 ...
	 */
	function getArticleRouteDO($id, $catid = 0, $sectionid = 0, $alias='', $catalias='')
	{
		$needles = array(
			'article'  => (int) $id,
			'category' => (int) $catid,
		...
		//Create the link
		$link = 'index.php?option=com_content&view=article&id='. $id.($alias?':'.$alias:'');

		if($catid) {
			$link .= '&catid='.$catid.($catalias?':'.$catalias:'');
		}
	...
	function getCategoryRouteDO($catid, $sectionid, $catalias)
	{
		$needles = array(
			'category' => (int) $catid,
			'section'  => (int) $sectionid
		...
		//Create the link
		$link = 'index.php?option=com_content&view=category&id='.$catid.':'.$catalias;
