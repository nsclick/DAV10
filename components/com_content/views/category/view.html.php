...
class ContentViewCategory extends ContentView
{
	function display($tpl = null)
	{
		...
		$access->canPublish		= $user->authorize('com_content', 'publish', 'content', 'all');
			 ...
			 */
			// parametros - keywords
			$sparams = new JParameter( $category->params, JPATH_COMPONENT_ADMINISTRATOR.DS.'parametros.xml', 'params' );
					...
					case 'imagenes' :
						if( count( $items ) ) :
							foreach( $items as $a => $item ) :
								...
								$patronImg = "(<img[^<>]*/>)";
								if ( ereg( $patronImg, $item->introtext, $regs ) ) :
									$img 			= $regs[1];
									$patronStyle 	= "( style=\"[^<>]*\")";
									...
									$items[$a]->img			= $img;
									$items[$a]->introtext	= ereg_replace($patronImg,"",$item->introtext);
								endif;
							endforeach;
						endif;
					break;
		...
		$this->assignRef('DOparams',	$sparams);
