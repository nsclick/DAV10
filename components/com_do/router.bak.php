...
function DoBuildRoute(&$query) {
	...
	switch( $query['c'].$query['task'] )
	{
		case 'categorias' :
			...
			$row		=& JTable::getInstance('categorias', 'DO');
			$row->load( $query['id'] );
			$segments[] = $row->id.':'.JFilterOutput::stringURLSafe($row->alias?$row->alias:$row->name);
		break;
		case 'productos' :
...
function DoParseRoute($segments){
    $vars = array();
