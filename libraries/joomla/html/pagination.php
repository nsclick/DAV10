...
class JPagination extends JObject
{
	 ...
	 * @since	1.0
	 */
	function getPagesLinksComponente()
	{
		global $mainframe;
		...
		// Build the page navigation list
		$data = $this->_buildDataObjectComponente();
	 ...
	 * @since	1.0
	 */
	function getPaginadorComponente()
	{
		global $mainframe;
		...
		$list['limitfield']		= $this->getLimitBox();
		$list['pagescounter']	= $this->getPagesCounter();
		$list['pageslinks']		= $this->getPagesLinksComponente();
	 ...
	 * @since	1.5
	 */
	function _buildDataObjectComponente()
	{
		global $Itemid;
		...
		if (!$this->_viewall) {
			$data->all->base	= '0';
			$data->all->link	= JRoute::_("&option=$option&Itemid=$Itemid&limitstart=");
		}
		...
		if ($this->get('pages.current') > 1)
		{
			...
			$data->start->base	= '0';
			$data->start->link	= JRoute::_("&option=$option&Itemid=$Itemid&start=0");
			$data->previous->base	= $page;
			$data->previous->link	= JRoute::_("&option=$option&Itemid=$Itemid&start=".$page);
		}
		...
		if ($this->get('pages.current') < $this->get('pages.total'))
		{
			...
			$data->next->base	= $next;
			$data->next->link	= JRoute::_("&option=$option&Itemid=$Itemid&limitstart=".$next);
			$data->end->base	= $end;
			$data->end->link	= JRoute::_("&option=$option&Itemid=$Itemid&limitstart=".$end);
		}
		...
		for ($i = $this->get('pages.start'); $i <= $stop; $i ++)
		{
			...
			{
				$data->pages[$i]->base	= $offset;
				$data->pages[$i]->link	= JRoute::_("&option=$option&Itemid=$Itemid&limitstart=".$offset);
			}
		}
		return $data;
	}
