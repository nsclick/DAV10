...
class modAutoconsultaHelper
{
	function getDatos( &$params )
	{
		$datos					= new stdClass;
		$datos->titulo			= $params->get('titulo','');
		$datos->descripcion		= $params->get('autoconsulta') && !IPprivada() ? nl2br($params->get('descripcion_offline','')) : nl2br($params->get('descripcion',''));
		...
		if( $params->get('icono','') ) :
			...
			if( file_exists( $icono ) ) :
				$datos->icono	= '<img src="'.JURI::base().'images/boxs/'.$params->get('icono','').'" alt="'.$datos->titulo.'" title="'.$datos->titulo.'" align="right" border="0" />';
			endif;
		endif;
