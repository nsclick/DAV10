...
class UploadHandler
{
    ...
    protected $error_messages = array(
        ...
        'min_width' => 'La imagen excede el ancho mínimo',
        //'max_height' => 'Image exceeds maximum height',
        'max_height' => 'La imagen excede el alto máximo',
        //'min_height' => 'Image requires a minimum height'
        'min_height' => 'La imagen excede el alto minimo'
    );
    ...
    protected function get_user_path() {
        if ($this->options['user_dirs']) {
            if($this->options['name_user_dir']) {
                return $this->get_name_user_dir().'/';
            } else {
    ...
    protected function get_name_user_dir() {
        return $this->options['name_user_dir'];
    }

    protected function get_rename_file() {
        return $this->options['rename_file'];
    }

    protected function get_read_folders() {
        return $this->options['read_folders'];
    }
    ...
    protected function get_upload_path($file_name = null, $version = null) {
        ...
        if(strstr($file_name, '/')) {
            ...
            $ruta = $ex[0];
            (!empty($version_path)) ? $ruta .= '/' . $version_path : $ruta .= '/';
            $ruta .= $ex[1];
        } else {
            $ruta = $version_path.$file_name;
        }

        return $this->options['upload_dir'].$this->get_user_path().$ruta;
    }
    ...
    protected function get_download_url($file_name, $version = null, $direct = false) {
        ...
        if($this->get_read_folders()){
            if(strstr($file_name, '/')) {
                $aux = explode('/', $file_name);
                return $this->options['upload_url'].$this->get_user_path().rawurlencode($aux[0]).'/'.$version_path.rawurlencode($aux[1]);
            } else {
                $aux = explode('.', $file_name);
                return $this->options['upload_url'].$this->get_user_path().rawurlencode($aux[0]).'/'.$version_path.rawurlencode($file_name);
            }
        }
    ...
    protected function is_valid_folder_object($nombre) {
        $folder_path = $this->get_upload_path($nombre);
    ...
    protected function get_folder_object($nombre) {

        $file_name =  $nombre . '.jpg';
        $loc_file_name = $nombre . '/' . $file_name;

        if($this->is_valid_folder_object($nombre)) {
            $file = new stdClass();
            $file->name = $file_name;
            $file->size = $this->get_file_size($this->get_upload_path($loc_file_name));
            $file->url = $this->get_download_url($file->name);
            foreach($this->options['image_versions'] as $version => $options) {
                if (!empty($version)) {
                    if (is_file($this->get_upload_path($loc_file_name, $version))) {
    ...
    protected function get_folder_objects($iteration_method = 'get_folder_object') {
        ...
        foreach (scandir($upload_dir) as $re) {
            if ($re === '.' || $re === '..') continue;
            if (is_dir($upload_dir . '/' . $re)) $arr[] = $re;
        }
        return array_values(array_filter(array_map(array($this, $iteration_method), $arr)));
    }
    ...
    protected function create_scaled_image($file_name, $version, $options) {
        ...
        if (!empty($version)) {
            if($this->get_read_folders()) {
                $ruta_archivo = $this->get_upload_path($file_name, $version);
                $aux = explode('/', $ruta_archivo);
                array_pop($aux);
                $aux = implode('/', $aux);
                $version_dir = $aux;
            } else {
                $ruta = null;
                $version_dir = $this->get_upload_path($ruta, $version);
            }
            ...
            if($this->get_read_folders()) {
                $ex = explode('/', $file_name);
                $new_file_path = $version_dir.'/'.$ex[1];
            } else {
    ...
    protected function get_unique_filename($name, $type = null, $index = null, $content_range = null) {
        ...
        ($this->get_read_folders()) ? $name = explode('.', $name)[0] . '/' . $name : $name = $name;
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            ($this->get_read_folders()) ? $ruta = $file->name : $ruta = '';
            /**/
            $ruta_archivo = $this->get_upload_path($ruta);
            $aux = explode('/', $ruta_archivo);
            array_pop($aux);
            $upload_dir = implode('/', $aux);
            /**/
    ...
    protected function generate_response($content, $print_response = true, $fromPost = false) {
        
        if(!$this->options['print_result']) {
            if(!$fromPost) {
    ...
    public function get($print_response = true) {
        ...
        if ($file_name) {
            ...
            if($this->get_read_folders()) {
                $response = array(
                    $this->options['param_name'] => $this->get_folder_objects()
                );
            } else {
                $response = array(
    ...
    public function post($print_response = true) {
        ...
        if($this->get_rename_file()) {
            $file_name = $this->get_rename_file();
        } else {
        ...
        return $this->generate_response(array($this->options['param_name'] => $files), $print_response, true);
    }
