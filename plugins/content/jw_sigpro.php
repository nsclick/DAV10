...
function jwSIGPRO( &$row, &$params, $page=0 ) {
    ...
    if ( $count ) {
		...
		// load the plugin language file in the frontend
		...
		include_once($mosConfig_absolute_path."/plugins/content/{$plg_name}/popup_engines/{$popup_engine}/popup.php");
...
<style type=\"text/css\" media=\"all\">
	@import \"{$mosConfig_live_site}/plugins/content/{$plg_name}/templates/{$thb_template}/template.css\";
</style>
<!--[if IE 7]>
<style type=\"text/css\" media=\"all\">
	@import \"{$mosConfig_live_site}/plugins/content/{$plg_name}/templates/{$thb_template}/template_ie7.css\";
</style>
<![endif]-->
<!--[if lte IE 6]>
<style type=\"text/css\" media=\"all\">
	@import \"{$mosConfig_live_site}/plugins/content/{$plg_name}/templates/{$thb_template}/template_ie6.css\";
</style>
<![endif]-->
{$plg_copyrights_end}
		";
		...
		if (preg_match_all($regex, $row->text, $matches, PREG_PATTERN_ORDER) > 0) {
			...
			foreach ($matches[0] as $key => $match) {
				...
				$plg_html = $plg_copyrights_start.'<ul id="sig'.$galleryID.'" class="sig-container">'.$showGallery.'<li class="sig-clr">&nbsp;</li></ul>'.$plg_copyrights_end;
