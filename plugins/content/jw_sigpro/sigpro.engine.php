...
if(!function_exists('jwGallery')){
	function jwGallery(
		...
		if ($dir = @ opendir($site_absolutepath.$srcimgfolder)) {
			...
			if ($found) {
				...
				//foreach($found as $filename){
					...
					$real_variables = array(
						...
						$site_httppath.$thbimgfolder.$prefix.replaceWhiteSpace(substr(md5($srcimgfolder),1,10).$filename),
					...
					$thumbimage = $site_absolutepath.$thbimgfolder.$prefix.$filename;
			...
			$result = JText::_('JWSP_MSG_ERRFOLDER');
		}
