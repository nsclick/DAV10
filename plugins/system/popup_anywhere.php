...
class plgSystemPopup_Anywhere extends JPlugin{
	...
	function onAfterDispatch(){
		...
		$js = "
			SqueezeBox.fromElement('".$this->_name."');
		...
		$document->addScriptDeclaration("
				window.addEvent('domready', function() {
					var archor = new Element('a', {
						'style':'display:none',
						'rel':'{".addslashes($options)."}',
						'href':'".$link."',
						'title':'".JText::_("MOOTOOLS12")."',
						'id':'".$this->_name."'
					}).inject($(document.body));
					archor.innerHTML='".JText::_("MOOTOOLS12")."';
				});
	...
	function _wrapJS($js, $onload=false, $globals=''){
		...
		return $globals.'
			window.addEvent("domready", function() {'.$js.'
		});';
	}
